package chapter06.daniel_liang;
...
public class DL_Exercise_034 {
	...
	public static void printFirstDay(int month, int year) {
		...
		String leadingTabs = "1"; // Holds any leading tabs to align 1st row of numbers in a calendar. This takes
									// care of firstDay=1

		// cases for firstDay between 2 & 6 (adds a "\t" at the beginning of the string
		// each iteration)
		// Loop starts from 1 since we want 1 less tab than the value of firstDay
		// (firstDay=1 is 0 tabs, firstDay=2 is 1 tab, firstDay=3 is 2 tabs, firstDay=4
		// is 3 tabs, firstDay=5 is 4 tabs)
		for (int i = 1; i < firstDay; i++) {
			leadingTabs = "\t" + leadingTabs;
		}
		if (firstDay == 0) { // reset it & ignore what the loop did if it's 0. THIS IS A SPECIAL CASE. We
								// want 6 tabs
			leadingTabs = "\t\t\t\t\t\t1";
		}
