 ...
 * (closes * it if it was open, and opens it if it was closed). Student S4
 * begins with * locker L4 and changes every fourth locker. Student S5 starts
 * with L5 and * changes every fifth locker, and so on, until student S100
 * changes L100. * * After all the students have passed through the building and
 * changed the * lockers, which lockers are open? Write a program to find your
 * answer and * display all open locker numbers separated by exactly one space.
...
public class DL_Exercise_023 {
	...
	/**
	 * studentLockerChanges changes the string in each element from "CLOSED" to
	 * "OPEN" or Vice versa
	 */
