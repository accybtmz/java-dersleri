 ...
 * whether three points are on the same line. * Write the following method to
 * test whether all the points in the array points * are on the same line. * *
 * public static boolean sameLine(double[][] points) * * Write a program that
 * prompts the user to enter five points and displays * whether they are on the
 * same line.
 */
public class DL_Exercise_015 {
	...
	public static boolean sameLine(double[][] points) {
		for (int i = 0; i < points.length - 2; i += 3) {
			if (pointPosition(points[i][0], points[i][1], points[i + 1][0], points[i + 1][1], points[i + 2][0],
					points[i + 2][1]) != 0) {
				return false;
			}
		}
